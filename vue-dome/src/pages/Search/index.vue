                ...
                <li :class="{ active: orderArr[0] === '1' }" @click="setOrder('1')">
                  <a href="javascript:"
                    ...
                    <i
                      class="iconfont"
                      :class="
                        orderArr[1] === 'desc' ? 'iconshang-copy' : 'iconshang'
                      "
                      v-if="orderArr[0] === '1'"
                    ></i>
                  </a>
...
export default {
  ...
  methods: {
    ...
    addProp(prop) {
      ...
      props.push(prop); // 向props数组添加一个条件字符串   子向父传递:自定义事件
      this.getShopList(); // 重新发请求获取列表
    },
    //删除一个属性条件
    removeProp(index) {
      // 删除props中的index 元素
